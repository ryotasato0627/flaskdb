...
def get_all_notes():
    ...
    if not user_id:
        error_resoponse("ログインしてください", 401)
...
def create_note():
    ...
    user_id = session.get("user_id")
    if not user_id:
        return error_resoponse("ログインしてください", 401)
    if not data or "title" not in data or "content" not in data:
        return error_resoponse("titleとcontentは必須です", status=400)
...
def update_note(id):
    ...
    if not data or not "title" in data or not "content" in data:
        return error_resoponse("titleとcontentは必須です", e)
    note.title = data.get("title", note.title)
    note.content = data.get("content", note.content)
    db.session.commit()
    logger.ingo(f"{id}のNoteを更新しました")
